...
# ⛔ **형상 그리드를 고칠 때 이 목록도 함께 봐라.** 2026-08-28 에 5090 의
#    below_launch_overhead 문턱(12.29 us) 아래라는 이유로 그리드에서 512³ 를
#    뺐는데 **이 목록은 그대로 뒀다.** 그래서 앵커가 문턱 바로 위(14.3 us,
#    문턱의 117 %)에서 측정됐고, 그 앵커 하나가 G-7 세 항목을 실패시켰다.
#    측정값이 12.288 / 14.336 us 를 오갔는데 각각 브래킷 오버헤드(4.096 us)의
#    정확히 3.0 배와 3.5 배다 — **런치 오버헤드에 지배되는 구간**이다.
DRIFT_SHAPES = [
    # 짧은 쪽. 드리프트 감시의 민감도가 여기서 나온다.
    #
    # 드리프트는 **런치당 상수**(5090 에서 문턱 위 +251 us)이므로 민감도는
    # 대략 251/t 다. 그래서 무작정 짧을수록 좋은 것이 아니라, **문턱 위이면서
    # 충분히 짧은** 지점을 골라야 한다:
    #
    #     512³   ~14 us   +1750 %   ⛔ 런치 오버헤드 구간. 값이 이산적으로 튄다
    #     2048³  ~73 us   ★ +344 %  문턱의 6 배. 넘는 순간 즉시 보인다
    #     4096³  ~940 us  +27 %     ⛔ A6000 이 실패한 지점
    Problem(2048, 2048, 2048),
    # 긴 쪽. 계산 처리율 회귀를 본다. **감시 판정에는 쓰지 않는다**
    # (DRIFT_MONITOR_SHAPE 참조).
