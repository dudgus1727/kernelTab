...
## 수정 목록 — ✅ 11 건 전부 완료 (2026-08-20)

> 컨테이너에서 실제로 돌려 확인했다. **호스트에서 268 개 테스트가 전부
> 통과하는 상태였는데도 여섯 개가 더 나왔다** — 자세한 것은
> `docs/decisions.md` 13 의 "컨테이너가 여섯 개를 더 찾았다".
>
> 7 번은 **전제가 틀렸다.** "pandas 제거" 라고 적혀 있으나 `core/table.py`
> 와 `core/bundle.py` 가 쓴다. 제거가 아니라 필수화했다.

## (원래) 수정 목록 (측정 완료 후 일괄 적용)
