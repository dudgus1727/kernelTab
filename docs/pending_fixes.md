...
## R-7. 앵커 하나가 **쌍봉**이라 노이즈 계수를 2.45 배 부풀린다

G-7 이 "측정 노이즈가 모델의 17.4 배" 로 경고한 앵커를, 전수 뒤 **이 캠페인의
계수**(A6000 이 아니라 H100 앵커 5,448 줄)로 다시 재니 **19.9 배**로 남았다.
같이 경고된 둘(4.1 / 3.8 배)은 1.6 / 1.3 배로 내려갔다 — **그 둘은 A6000
계수 탓이었고, 이것은 아니었다.**

### 노이즈가 아니라 쌍봉이다

```
sm90_tb256x256x32_w128x64x32_st6 @ 4096   454 회
   2.60~2.74 ms  약 219 회
   ★ 2.74~2.84 사이가 비어 있다
   2.87~2.89 ms  약 227 회
   차이 +9.37%   sd 4.27%   (클럭 딥은 1 회뿐 — 전력이 아니다)
```

★ 이 커널이 `decisions` 31 의 그 커널이다. 다른 커널
(`tb256x256x64_w128x64x64_st3_swid8`)이 같은 프로세스에서 먼저 돌면 그 뒤로
빨라진다. 앵커는 세그먼트 프로세스마다 다시 재므로 **어떤 프로세스는 전이
후, 어떤 프로세스는 전이 전** 상태에서 쟀다.

**G-7 의 "17.4 배" 와 G-7 5 번 실패가 같은 원인이었다.** 그때는 몰랐다.

### 계수에 미친 영향

`sigma_rel` 은 긴 절반 6 개의 상대 sd **중앙값**인데 이 앵커(4.27 %)가 중앙
위치를 한 칸 밀었다.

```
              sigma_abs      sigma_rel
전체 (현재)   4.303e-04      2.001e-03
쌍봉 제외     4.078e-04    ★ 8.158e-04     2.45 배 작다

바닥   t=0.5 ms  0.286% -> 0.163%
       t=2.85    0.215% -> 0.096%
       t=24.1    0.202% -> 0.083%
```

긴 형상에서 바닥이 **2.4 배 넓게** 잡혀 있다. 정답 집합이 그만큼 관대하다.

### ⛔ 지금 고치지 마라 — 그리고 "앵커를 빼는" 것도 답이 아니다

```
⛔ 계수를 바꾸면 번들의 noise_floor 가 바뀐다. 이미 릴리즈됐고 소비 쪽이
   bundle_guard 로 그 값을 검증한다
⛔ "이 앵커를 빼면 좁아진다" 는 앵커를 골라내는 것이다 — decisions 29 의
   금지 목록 "실패한 조합을 뺀다" 와 같은 형태다
```

★ 정당한 수정은 셋 중 하나다.

1. 전이의 **기전을 밝혀** 조건을 없앤다 (`decisions` 31 의 "확인하지 않은
   것" 부터 — L2 설정 / persisting window / smem carveout / 클러스터).
2. 앵커를 **프로세스 상태에 무관하게** 재도록 만든다 (예: 세그먼트 시작에
   모든 앵커 커널을 한 번 돌려 상태를 맞춘다 — `recheck` 에 넣은 예열과 같은
   처방).
3. 계수 추정을 **쌍봉에 강건하게** 바꾼다 (중앙값 대신 최빈 구간 등).
   ⚠️ 3 번만 하면 쌍봉 자체는 남고 그 앵커의 값은 여전히 두 값이다.

**다음 캠페인 전에** 정한다. 지금 표를 재판정하면 네 표의 비교 가능성이
깨진다.
