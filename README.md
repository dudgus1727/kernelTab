...
> |---|---|---|
> | 문턱 (모듈 수) | **~1,200 개** (그 아래는 증가 0) | `segments.kernels` 를 이 값의 절반 이하로 (A6000: 500) |
> | 상수 성분 | **75 us / 모듈 1,000 개** (문턱 위) | 허용 오차에서 세그먼트 크기 역산 |
> | 짧은 커널 왜곡 배율 | 100 배 (15 us vs 22 ms) | 감시 프로브에 짧은 형상이 필요한 이유 |
>
> 절차: **`docs/new_environment_checklist.md`** (게이트 8개, 선행 ~7시간).
...
1485~1935 MHz 를 오갔다. 클럭 고정 후(`env_hash = 368a84f1...`)의 전수 데이터와
**절대 시간을 직접 비교할 수 없다.** 조인/필터할 때 반드시 `env_hash` 로 나눠라.

**클럭 고정의 효과** (리허설 조건, 조합 20~30개 x 소수 회 재측정):
...
> 이 표는 **클럭 고정 전후를 비교하는 용도**다. 리허설 조건
> (`b42df475` / `de6de3c1`)에서 잰 것이고 표본이 작다. **공개하는
> 데이터셋의 재현성이 아니다** — 그건 아래를 봐라.

### 재현성 — 공개 데이터셋(`c63710df`) 기준, **형상 크기별로 다르다**

33시간 캠페인 **자체에 앵커를 심어** 잰 값이다. 조합 12개 x **312회**
재측정, 프로세스 104회 재시작을 가로지른다 (`results/anchors.jsonl`).

| 커널 시간 | 상대 표준편차 | 33시간 변동폭 | 1% 차이를 구분할 수 있나 |
|---|---:|---:|---|
| **> 1 ms** (6조합, 1.50~2.92 ms) | **0.044%** | 0.11 ~ 1.41% | **가능** |
| **< 0.1 ms** (6조합, 14~56 us) | **1.541%** | 1.82 ~ 7.14% | **불가능** |

**35배 차이다.** 단일 숫자로 뭉개면 안 된다 — 15 us 커널에서 1 % 는
재현되지 않는 차이라서, 고정 1 % 허용치를 쓰면 그 형상에서 **노이즈를
신호로 학습**하게 된다.

그래서 노이즈 바닥을 **형상별 함수**로 둔다 (`core/noise.py`).

```
sigma_rel(t) = 0.000374 / t + 0.00044      # 절대 지터 + 상대 성분

  t = 0.015 ms -> 2.66%      1% 구분 불가
  t = 0.100 ms -> 0.42%      불가
  t = 0.500 ms -> 0.12%      가능 (여기가 경계)
  t = 2.916 ms -> 0.06%      가능
```
...
같은 결론이 세 방향에서 나왔다 — 난이도(M=1/8 형상 1.11~1.15), 순위
안정성(`1x12288x4096` 은 5 % 이내 config 가 2,501개), 그리고 이 재현성.
세 번째가 앞 둘의 **원인 일부**다.

### ⛔ `env_hash = 368a84f1...` 의 226,211 행은 분석에서 제외한다 (D-5)

클럭을 고정한 뒤 돌린 첫 전수 측정(2026-08-16 10:16 ~ 17:47, 전체의 23%)은
**측정 드리프트 구간**이다. 같은 커널·같은 형상의 측정값이 7.5 시간에 걸쳐
**+5.06%** 느려졌다. 클럭도 온도도 스로틀도 정상이었다 — 원인은
`docs/measurement_drift.md` 에 있다.

이 데이터로 config 를 비교하면 **먼저 측정된 config 가 이긴다.** 셔플이
편향을 무작위화하지만 없애지는 못한다. 5% 는 config 간 성능 차이와 같은
크기다.

* 파일에서 지우지 않는다 — `results/*.jsonl` 은 append-only 다.
  드리프트 자체가 연구 대상이고, 이 행들이 그 증거다.
* `env_hash` 로 구분된다. 대책을 넣은 재측정은 **다른 `env_hash`** 를 갖는다.
* 소비 쪽(`core/table.py`, `core/bundle.py`)은 `env_hash` 로 필터하므로
  섞이지 않는다. 번들에는 재측정본만 들어간다.

| `env_hash` | 행 | 구간 (UTC) | 조건 | 상태 |
|---|---:|---|---|---|
| `b42df475...` | 1,526 | 08-16 07:46~07:50 | 클럭 미고정, 리허설 | 참고용 |
| `de6de3c1...` | 834 | 08-16 08:51~08:54 | 클럭 고정, 리허설 | 참고용 |
| `368a84f1...` | 226,211 | 08-16 10:16~17:47 | 클럭 고정, 드리프트 대책 없음 | **분석 제외** |
| (재측정) | — | — | 클럭 고정 + 드리프트 대책 | 사용 |
...
으로 따로 배포한다. 데이터셋에 소프트웨어 라이선스를 붙이면 이용자가 오히려
혼란스러워진다.

번들은 저장소와 분리되어 유통되므로 **파일이 라이선스를 들고 다닌다** —
`BUNDLE.json` 의 `license` / `tool_license` 필드와 번들 안 `LICENSE.txt`.
