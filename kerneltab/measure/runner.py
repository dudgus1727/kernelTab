...
#: ★ **측정 경로 리비전.** 측정된 시간을 바꾸는 변경이 있으면 사람이 올린다.
#:
#: `env_hash` 는 `manifest_hash`(소스 tree_hash)를 일부러 안 넣는다 — 오타
#: 수정에도 바뀌어 측정 중 아무것도 못 고치게 되기 때문이다. 그 대가로
#: **측정 경로를 고쳐도 해시가 안 바뀌는 구멍**이 생겼고, 문서는 "조건이
#: 달라졌으면 phase0 를 다시 돌려라" 라고 적어 두었다. 그런데 해시는 바뀌지
#: 않은 입력들의 순수 함수라 **다시 돌려도 같은 값이 나온다** — 그 지시는
#: 실제로는 아무 일도 하지 않았다 (2026-09-03 H100 에서 드러났다).
#:
#: 그래서 굵은 입도의 명시적 값을 하나 둔다. tree_hash 처럼 예민하지 않고,
#: 규율이 아니라 해시 입력이다.
#:
#: | 값 | 바뀐 것 |
#: |---|---|
