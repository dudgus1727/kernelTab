...
#
# 5, 7 은 **축 덮개 점검이 찾아낸 구멍**이다 (2026-08-28, RTX 5090).
# 벤더 휴리스틱이 594 개 추천 중 92 개(15.5 %)를 이 둘로 냈는데 우리 축에
# 없었다. 3, 6, 12 는 3배수 가설 때문에 명시적으로 넣었고 2의 거듭제곱은
# 당연히 넣었는데, 5 와 7 은 **어느 쪽 이야기에도 안 들어가서 아무도 안
# 물었다.** 누락이지 결정이 아니었다.
#
# 근거 (docs/axis_coverage.md):
#   유효성  67 형상 중 54 개에서 "요청 == 실제 슬라이스" — 이미 축에 있던
#           split_k=12 (14 형상)보다 훨씬 넓다
#   수치    serial / parallel 둘 다, 서로 다른 커널 2 종으로 참조 GEMM 과
#           대조해 정확히 일치. `can_implement()` 통과만으로 끝내지 않았다
#           (stages=1 이 그것을 통과하고도 틀렸다 — decisions.md 13-b)
#
# ⚠️ A6000 캠페인의 축은 5, 7 이 없는 8 개짜리다. 전이 실험에서
#    common_shapes_only 와 함께 **config 공간도 교집합을 잡아야 한다.**
SPLIT_K = [1, 2, 3, 4, 5, 6, 7, 8, 12, 16]
